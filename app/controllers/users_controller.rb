...
#    skip_before_action :authorize, only: [:create]
   ...
   def create
    ...
    user = User.create!(name: params[:name], age: params[:age], password: params[:password], bio: params[:bio], organization_id: params[:organization_id], address: params[:address], city: params[:city], state: params[:state], zip: params[:zip], avatar: params[:avatar], lng: lng, lat: lat)
   ...
   def user_params
      params.permit(:name, :age, :password, :password_confirmation, :bio, :organization_id, :address, :city, :state, :zip, :avatar)
   end
end
